...
const prepareOutputFilename = (inputName) => {
  const inputElementsArray = inputName.split('.');
  const inputNameWithWatermarks = inputElementsArray[0] + '-with-watermark';
  const inputType = inputElementsArray[1];
  return inputNameWithWatermarks + '.' + inputType;
};
